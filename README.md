...
* **高级编译优化**：JIT 引擎现在在生成三地址码 (3AC) 时，会结合 CFG 分析进行局部的**常量折叠与传播 (Constant Folding)**、**代数恒等式替换 (Strength Reduction)** 以及**死分支消除 (Dead Branch Elimination)**。大量冗余计算被直接在编译期裁剪，进一步推高了运行期性能上限。
